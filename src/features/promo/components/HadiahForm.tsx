  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col h-[750px] -m-2">
      <div className="flex-1 overflow-y-auto p-4 space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
           <FormField label="Nama Program Promo" icon={Zap} required className="md:col-span-2">
              <Input 
                 ...
                 className="h-11 bg-card font-bold uppercase tracking-tight rounded-xl placeholder:opacity-30" 
                 required 
              />
           </FormField>
           ...
           {isSuperOrCompanyAdmin && (
             <>
               <FormField label="Target Cluster" icon={Tag}>
                 ...
                 >
                   <SelectTrigger className="h-10 bg-card font-bold text-sm rounded-xl">
                 ...
                 <Popover>
                    <PopoverTrigger asChild>
                        <div className="relative group cursor-help">
                            ...
                            >
                                <SelectTrigger className="h-10 bg-card font-bold text-sm rounded-xl disabled:opacity-50">
           ...
           <FormField label="Berlaku s/d" icon={CalendarIcon} required>
                <Popover>
                    <PopoverTrigger asChild>
                        <Button variant="outline" className="w-full h-10 justify-start text-sm font-bold bg-card rounded-xl">
        ...
        <div className="bg-muted/30 p-5 rounded-2xl border border-border space-y-5">
           <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                 <div className="p-1.5 bg-primary/10 rounded-lg">
                    <ArrowRight className="h-4 w-4 text-primary" />
                 </div>
                 <span className="text-xs font-black uppercase tracking-widest text-muted-foreground">Pemicu Promo</span>
              </div>
           ...
           {formData.jenis_pemicu === 'produk' ? (
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
                 <div className="md:col-span-3">
                    <FormField label={`Pilih Produk Pemicu (${selectedPemicuIds.length})`} icon={Package} required>
                        <Popover open={isPemicuPopoverOpen} onOpenChange={setIsPemicuPopoverOpen}>
                            <PopoverTrigger asChild>
                                <Button variant="outline" className="w-full h-10 justify-between bg-white font-bold text-sm rounded-xl">
                                ...
                                <ScrollArea className="h-60">
                                    <div className="p-2 space-y-1">
                                        {produks.map(p => {
                                            ...
                                            return (
                                                <div key={p.id} className={cn("flex items-center gap-3 p-3 rounded-xl transition-all", isSelected ? "bg-primary/5" : "hover:bg-muted/50 cursor-pointer")} onClick={() => handleTogglePemicu(p)}>
                                                    ...
                                                    <div className="flex flex-col flex-1 min-w-0">
                                                        ...
                                                        <span className="text-[9px] uppercase font-black opacity-30">{p.sku}</span>
                                                    </div>
                                                </div>
                                            );
                                        })}
                 ...
                 <div>
                    <FormField label="Min Qty" icon={Layers}>
                        <Input type="number" value={formData.min_qty_pemicu} onChange={(e) => setFormData({...formData, min_qty_pemicu: e.target.value})} className="h-10 bg-white font-black text-center text-sm rounded-xl" />
                    </FormField>
                 </div>
              </div>
           ) : (
              <FormField label="Minimum Total Pembayaran Per Nota (Rp)" icon={Coins}>
                 <div className="relative">
                    <Input type="number" value={formData.min_amount_pemicu} onChange={(e) => setFormData({...formData, min_amount_pemicu: e.target.value})} className="h-11 bg-white font-black pl-10 text-base rounded-xl border-emerald-200" />
        ...
        {/* Gift / Hadiah Section */}
        <div className="bg-emerald-50/50 p-6 rounded-2xl border-2 border-dashed border-emerald-200 space-y-5 animate-in fade-in slide-in-from-bottom-4">
           ...
           <div className="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
              <div className="md:col-span-2">
                 <FormField label="Pilih Produk Hadiah" icon={Package} required>
                    <Popover open={isHadiahPopoverOpen} onOpenChange={setIsHadiahPopoverOpen}>
                        <PopoverTrigger asChild>
                            <Button variant="outline" className="w-full h-11 justify-between bg-white border-emerald-100 font-bold text-sm rounded-xl">
                                ...
                                <div className="p-2 space-y-1">
                                    {produks.map(p => (
                                        <div key={p.id} className="flex items-center gap-4 p-3 rounded-xl hover:bg-emerald-50 cursor-pointer" onClick={() => handleSelectHadiah(p)}>
                                           ...
                                           <div className="flex flex-col flex-1 min-w-0">
                                              <span className="font-bold text-[12px] truncate">{p.nama_produk}</span>
                                              <span className="text-[9px] uppercase font-black opacity-30 italic">{p.sku}</span>
                                           </div>
                                        </div>
                                    ))}
                                </div>
              ...
              <div>
                <FormField label="Qty" icon={Layers}>
                    <Input type="number" value={formData.qty_hadiah} onChange={(e) => setFormData({...formData, qty_hadiah: e.target.value})} className="h-11 bg-white border-emerald-100 font-black text-center text-sm rounded-xl" />
                </FormField>
              </div>
              ...
              <div>
                <FormField label="Harga Tebus" icon={Save}>
                    <div className="relative">
                        <Input type="number" value={formData.harga_tebus} onChange={(e) => setFormData({...formData, harga_tebus: e.target.value})} className="h-11 bg-white border-emerald-100 font-black pl-8 text-sm rounded-xl" />
                        <span className="absolute left-3 top-1/2 -translate-y-1/2 text-[10px] font-black text-emerald-400">RP</span>
                    </div>
                </FormField>
              </div>
           </div>

           <div className="p-3 bg-white/60 border border-emerald-100 rounded-xl flex items-center gap-3">
              <Info className="h-4 w-4 text-emerald-500" />
              <p className="text-[10px] font-semibold text-emerald-800/80 uppercase tracking-tight">
                 Harga tebus <span className="font-black text-emerald-600">RP 0</span> berarti hadiah diberikan secara <span className="font-black text-emerald-600 underline">GRATIS</span>.
              </p>
           </div>
        </div>
      </div>
      ...
      <div className="p-6 pt-4 border-t bg-muted/10 backdrop-blur-sm">
        <div className="flex justify-end gap-3">
          ...
          <Button 
            type="submit" 
            className="h-12 px-14 text-xs font-black uppercase tracking-widest shadow-2xl shadow-emerald-500/30 bg-emerald-600 text-white rounded-xl hover:scale-[1.02] active:scale-95 transition-all" 
